...
paymentsRouter.post('/webhook', async (req, res) => {
  try {
    ...
    if (!token || token !== process.env.MERCADO_PAGO_WEBHOOK_TOKEN) {
    ...
    if (metadata?.type === 'consulting_plan' && metadata.leadId) {
      if (status === 'approved') {
        try {
          const leadIdNum = Number(metadata.leadId);
...
<p>Seu pagamento do plano de consultoria <strong>${metadata.planId}</strong> foi confirmado com sucesso.</p>
